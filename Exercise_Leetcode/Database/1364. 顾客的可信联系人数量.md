...
## Solutions:
```
select c.invoice_id,e.customer_name,c.price
    ,isnull(d.contacts_cnt,0) contacts_cnt
    ,isnull(d.trusted_contacts_cnt,0) trusted_contacts_cnt
from Invoices c
left join (
    select user_id
        ,count(contact_email) contacts_cnt
        ,count(b.customer_name) trusted_contacts_cnt
    from Contacts a
    left join Customers b on a.contact_name=b.customer_name
    group by user_id
) d on c.user_id=d.user_id
left join Customers e on c.user_id=e.customer_id
order by c.invoice_id
```
执行用时 :1943 ms, 内存消耗 :0B
