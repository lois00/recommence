...
排序的时候需要注意把isnull()带上，否则如果第一个排序项b.num_points为null，第二个排序项就会失效。
```
select a.team_id,a.team_name,isnull(b.num_points,0) num_points
from Teams a
left join (
    select team_id,sum(goals) num_points 
    from (
...
order by isnull(b.num_points,0) desc,a.team_id asc
```
执行用时 :689 ms, 内存消耗 :0B
