...
对Deparment表进行多次自关联，关联条件除了部门id，还要指定每个month的值。这样关联以后就可以得到每个部门id从1月到12月的revenue。（使用左连接可以不用考虑null值的情况。）考虑到会有重复id的记录，使用max函数去重即可。
```
SELECT d.id
...
增加12个栏位，分别表示每个月的revenue。然后针对每条记录，判断month栏位值是哪个月份，然后把该记录的revenue值赋值给当月的revenue栏位，其他栏位默认为null。最后使用group by+sum函数去重即可。
```
SELECT id
