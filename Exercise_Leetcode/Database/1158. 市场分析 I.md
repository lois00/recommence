...
### 解法一：
```
select u.user_id buyer_id,u.join_date,isnull(t.orders_in_2019,0) orders_in_2019
from Users u
left join(
    select buyer_id,
...
```
执行用时 :2179 ms, 内存消耗 :0B


### 解法二：
```
select u.user_id buyer_id,max(u.join_date) join_date,count(o.order_id) orders_in_2019
from Users u
left join Orders o on u.user_id=o.buyer_id and year(o.order_date)=2019
group by u.user_id
```
执行用时 :2203 ms, 内存消耗 :0B
